...
function App(props) {
  return <SearchableCoinCrypitoList coins={props.coins} />;
}
...
function CoinCrypitoList(props) {
  ...
  coins.forEach((coin, index) => {
    if (coin.name.toLowerCase().indexOf(filterText) === -1) return;
    rows.push(
      <Draggable key={coin.name} draggableId={coin.name} index={index}>
        {(provided) => (
          <CoinRow
